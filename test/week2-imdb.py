...
eval_path = '../data/imdb/eval_data.txt'
test_path = '../data/imdb/test_data.txt'
...
class RNNModel(Model):
    def __init__(self, dict_size, word_dim, layer_num, hidden_size, output_num):  # 需要手动传dict_size 可优化
        ...
        self.embedding_layer.from_pretrained(torch.from_numpy(vecs))  # 扩展成3d？？？
        self.embedding_layer.weight.requires_grad = False

        self.rnn = torch.nn.LSTM(word_dim, hidden_size, layer_num)  # batch_first没用？？？
    ...
    def forward(self, X):
        # print('forward', X, X.shape)  # [500, 30]  # 此时X仍为2d？？？[batch, seq, wordvec]
        X = X.permute(1, 0)
...
if __name__ == '__main__':
    ...
    test_labels_path = '../data/output/imdb_v1_out.txt'
    ...
    # optimizer = torch.optim.Adam(rnnModel.parameters(), lr=1e-2)
